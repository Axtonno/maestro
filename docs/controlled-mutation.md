...
Il percorso v0.5.0 è qualificato sull’artifact Linux `amd64`, inclusa una
prova nativa CPU-only. Windows nativo e macOS non sono supportati. Il dettaglio
è nella pagina [Piattaforme supportate](supported-platforms.md).
...
I risultati pubblici sono sintetizzati in [Benchmark](benchmarks.md). Il
modello di sicurezza completo è in [Security Model](security-model.md).
